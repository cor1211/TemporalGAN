...
class Discriminator(nn.Module):
    ...
    """
    def __init__(self, s2_in_channels=3, lc_in_channels = 3, s1_out_channels=1, features=[64, 128, 256, 512]):
        """
        Args:
        -----
        ...
        lc_in_channels = lc_in_channels + s1_out_channels 
        ...
        self.lc_init_cov = CNNBlock(lc_in_channels, features[0], stride=2, norm=False) # we don't want batch/instance norm in the first layer.
        ...
        self.middle_lc = nn.Sequential(*layers)
    ...
    def forward(self, s2_in , lc_in, s1_out):
        """
        ...
        lc_in = torch.cat([lc_in, s1_out], dim=1) # Concatenating the output of the generator with the real s1 image
        
        s2_in = self.s2_init_cov(s2_in)
        lc_in = self.lc_init_cov(lc_in)
        
        s2_in = self.middle_s2(s2_in)
        lc_in = self.middle_lc(lc_in)
        
        concat = torch.cat([s2_in, lc_in], dim=1) # Concatenating the output of the two streams.
...
def test(summary=False, gpu=False):
    ...
    s2_in = torch.randn((1,3,256,256)).to(device)
    lc_in = torch.randn((1,3,256,256)).to(device)
    s1_out = torch.randn((1,1,256,256)).to(device)
    model = Discriminator(s2_in_channels=3, lc_in_channels=3, s1_out_channels=1).to(device)
    preds = model(s2_in, lc_in, s1_out)
    print(preds.shape)
    
    if summary:
        from torchinfo import summary
        summary(model, input_size=[(1, 3, 256, 256),(1, 3, 256, 256),(1,1,256,256)], device=device,col_names=["input_size", "output_size", "num_params"],
        col_width=20,
